...
export default function RetailBrandsPage() {
  ...
  const handleBrandSubmit = (e) => {
    ...
    alert(`Thank you, ${fullName}! Your registration request for "${brandName}" has been received.`);
    setFullName("");
    setEmail("");
    setBrandName("");
    setTargetCity("");
    ...
    <div className="bg-slate-50 min-h-screen font-sans antialiased text-slate-800">
      ...
      <section className="relative overflow-hidden bg-[#0B1F3A] min-h-[370px] flex flex-col justify-center">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-10 lg:px-12 lg:py-14">
          <p className="text-sm font-semibold tracking-[0.2em] text-[#E8A33D] uppercase">Verified Retail Portfolio</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 mt-8">
        <div className="bg-white rounded-2xl border border-gray-150 p-5 sm:p-6 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex-1 space-y-1 max-w-2xl text-left">
            <h4 className="font-serif font-semibold text-slate-950 text-lg tracking-wide">Register Brand Expansion Request</h4>
      ...
      <section className="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-6 flex-wrap gap-2">
          <h2 className="font-serif text-xl md:text-2xl font-normal text-slate-950 tracking-wide">Retail Sectors</h2>
            ...
            {CLASSIFICATION_COLUMNS.map((colItems, colIdx) => (
              <ul key={colIdx} className="space-y-3">
                {colItems.map((item, itemIdx) => {
                  runningItemCounter++;
                  return (
                    <li key={itemIdx} className="text-xs sm:text-sm text-slate-700 font-medium flex items-start gap-2.5 transition-transform duration-200 hover:translate-x-0.5">
                      <span className="text-slate-300 font-bold min-w-[20px] text-right">{runningItemCounter}.</span>
                      <span className="hover:text-[#c9a84c] cursor-pointer">{item}</span>
                    </li>
                  );
                })}
              </ul>
      ...
      <section className="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-slate-100">
        <div className="flex justify-between items-end mb-6">
          <h2 className="font-serif text-xl md:text-2xl font-normal text-slate-950 tracking-wide">Active Retail Brands</h2>
          <span className="text-xs font-bold text-[#c9a84c] uppercase tracking-wider">Verified Members</span>
        </div>
        ...
        <div className="relative">
          ...
          <div ref={brandsRef} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide scroll-smooth snap-x" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {BRANDS.map((sp) => (
              <Link
                key={sp.slug}
      ...
      <section className="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-slate-950 border border-slate-900 rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4 text-left">
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <h3 className="font-serif text-xl font-normal tracking-wide text-white">Have Premium Shopping Center Spaces?</h3>
      ...
      <section className="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-6">
          <h3 className="font-serif text-lg font-normal text-slate-950 tracking-wide">Store Openings</h3>
      ...
      <section className="pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white border border-gray-150 rounded-2xl p-5 sm:p-6 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4 text-left">
          <div className="shrink-0 max-w-xs">
            <h4 className="font-serif text-slate-950 font-normal text-lg tracking-wide">Retail Network Stats</h4>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3 flex-1 w-full">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mt-0.5">Vetted Brands</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mt-0.5">Flagship Assets</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mt-0.5">Consumer Footfall</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mt-0.5">Expansion Metros</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-white border border-slate-200 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
            ...
            <form onSubmit={handleBrandSubmit} className="p-6 space-y-4 text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5">Brand Name</label>
                  <input
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5">Target City</label>
                  <input
                    type="text" required value={targetCity} onChange={(e) => setTargetCity(e.target.value)}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1.5">Primary Segment</label>
                <select
